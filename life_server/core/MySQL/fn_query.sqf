...
switch (_side) do
{
	case west:
	{
		...
		*/
		_query = format["SELECT playerid, name, cash, bankacc, coplevel, cop_licenses, cop_gear, adminlevel, blacklist, donatorlvl, inventory, banned, playtime, perms, mediclevel FROM players WHERE playerid='%1'",_uid];
	};
	...
	case civilian:
	{
		...
		*/
		_query = format["SELECT playerid, name, cash, bankacc, civ_licenses, arrested, adminlevel, donatorlvl, civ_gear, inventory, banned, playtime, perms , dwlevel  FROM players WHERE playerid='%1'",_uid];
	};
};
