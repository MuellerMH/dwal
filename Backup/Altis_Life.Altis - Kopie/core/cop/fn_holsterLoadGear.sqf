...
_primaryWeaponMagazine = _loadout select 14;
_primaryWeaponItems = _loadout select 15;
_secondaryWeaponMagazine = _loadout select 16;
_secondaryWeaponItems = _loadout select 17;
_handgunItems = _loadout select 18;
...
if(_primary != "") then {[_primary,true,false,false,false] spawn life_fnc_handleItem;systemChat format["%1",_x];};
if(_launcher != "") then {[_launcher,true,false,false,false] spawn life_fnc_handleItem;systemChat format["%1",_x];};
if(_handgun != "") then {[_handgun,true,false,false,false] spawn life_fnc_handleItem;systemChat format["%1",_x];};

// {_handle = [_x,true,false,false,false] spawn life_fnc_handleItem; waitUntil {scriptDone _handle};} foreach _items;
{[_x,true,false,true,false] spawn life_fnc_handleItem;} foreach (_primaryWeaponMagazine);
{[_x,true,false,true,false] spawn life_fnc_handleItem;} foreach (_primaryWeaponItems);
{[_x,true,false,true,false] spawn life_fnc_handleItem;} foreach (_secondaryWeaponMagazine);
// {[_x,true,false,true,true] spawn life_fnc_handleItem;} foreach (_secondaryWeaponItems);
{[_x,true,false,true,false] spawn life_fnc_handleItem;} foreach (_handgunItems);
