...
if(_cops < 5) exitWith{[[_vault,-1],"TON_fnc_robberyState",false,false] spawn life_fnc_MP; hint "Es sind nicht genügend Cops anwesend um die Bank zu überfallen, ließ bitte die Regeln.";};
...
while {true} do
{
	...
	hintSilent format["Du musst maximal 30m Abstand halten!\n\nVerbleibende Zeit:\n %1\n\nDistanz: %2m",_countDown,round(player distance _vault)];

	if(player distance _vault > 30) exitWith {_toFar = true;};
...
switch(true) do
{
	...
	{
		hint format["Du hast erfolgreich $%1 geraubt!\n\nUm Rückverfolgung zu verhindern, kannst du 15 min lang keinen Bankautomaten benutzen.",[_funds] call life_fnc_numberText];
		[] spawn
		{
			life_use_atm = false;
			sleep 900;
		...
		life_cash = life_cash + (_funds + 5000000);
