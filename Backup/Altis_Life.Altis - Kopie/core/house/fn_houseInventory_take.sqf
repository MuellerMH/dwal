...
if(_item == "money") then
{
	_item_count = lbText  [3502, _selectedindex]; //get The Value String of Money
	_item_count = [_item_count,"$",""] call KRON_Replace;
	_item_count = parseNumber _item_count;
}
else
{
...
hint format["ItemCount =  %1",_item_count ]; sleep 4;
...
if(_count > _item_count) exitWith {
	hint format["Zu hohe Zahl eingegeben. Maximal %1",_item_count];
};
